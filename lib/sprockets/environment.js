...
ref$._createStream = function(mime_type){
  var targetExtention, collection, ref$, extEngines, dispatchStartStream, engineExtension, couldBeTargetExt, passThroughStream, dispatchEndStream, stream, this$ = this;
  ...
  dispatchStartStream._transform = function(file, enc, done){
    var extname, dispatchToStream, that, passThroughStream;
    extname = path.extname(file.path);
    dispatchToStream = (that = extEngines[extname])
      ? that
      : (that = this$.preprocessors[mime_type]) ? (passThroughStream = new PassThrough({
        objectMode: true
      }), that(this$, passThroughStream, dispatchStartStream), passThroughStream) : void 8;
    dispatchToStream.write(file);
    done();
  };
  for (engineExtension in ref$ = this.engine_extensions) {
    couldBeTargetExt = ref$[engineExtension];
    if (couldBeTargetExt !== targetExtention) {
      continue;
    }
    ...
    this.engines[engineExtension](this, passThroughStream, dispatchStartStream);
  }
