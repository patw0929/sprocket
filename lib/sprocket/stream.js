...
function SprocketStream(options){
  ...
  this._dispatchStream = createInternalStreams(this, options.extname, options.extensions || {});
}
...
ref$._transform = function(file, _, done){
  ...
  this._dispatchStream.write(file);
  done();
};
ref$.end = function(){
...
function createInternalStreams(stream, targetExtname, extensions){
  var _internalStreams, _dispatchStartStream, _targetEndStream, extname, configureFn, passThrough;
  _internalStreams = {};
  _dispatchStartStream = new Transform({
    objectMode: true
  });
  _dispatchStartStream._transform = function(file, _, done){
  ...
  for (extname in extensions) {
    ...
    configureFn(stream._environment, passThrough, extname === targetExtname ? _targetEndStream : _dispatchStartStream);
  }
  return _dispatchStartStream;
}