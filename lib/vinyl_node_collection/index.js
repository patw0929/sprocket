var util, path, NodeFinalizationError, Node, Edge, SuperNode, RequireState, Collection;
util = require('util');
path = require('path');
NodeFinalizationError = require('../errors/node_finalization_error');
Node = require('./node');
Edge = require('./edge');
...
Collection = (function(){
  ...
  function Collection(_default_unstable){
  ...
  prototype.createRequireStates = function(){
    ...
    for (keyPath in ref$ = this._nodes) {
      node = ref$[keyPath];
      if (node.hasAnyEdges) {
        results$.push(node.build_dependencies(new RequireState(keyPath, this)));
      }
    }
    return results$;
  };
...
function parseKeyPath(relative){
  ...
  if (relative.indexOf('.min') > -1) {
    ...
    if (relative.split('.').length - 1 == 1) {
      ...
      keyPath = relative.substring(0, relative.lastIndexOf('.'));
    } else {
      keyPath = ref$[0];
    }
...
Collection.prototype._createNodeWith = function(it){
  ...
  return (ref$ = this._nodes)[keyPath] || (ref$[keyPath] = new Node(keyPath, this._default_unstable));
};
